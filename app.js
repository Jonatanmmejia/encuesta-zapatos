...
async function sendToGoogleSheets(vote, url) {
    try {
        ...
        // La encuesta continúa normalmente aunque falle el envío
    }
}
...
function showResults() {
    ...
    // Show user's answers (solo las 9 primeras)
    html += '<div class="user-answers"><h3>Tus respuestas (Preguntas 1-9):</h3>';
    ...
    html += '<div class="overall-stats"><h3>Estadísticas Generales:</h3>';
    html += `<p class="stats-info">Total de votos registrados: <strong>${state.allVotes.length}</strong></p>`;

    // Calculate most popular options per question
    SURVEY_CONFIG.questions.forEach(question => {
        ...
        if (sorted.length > 0) {
            const topOptionId = parseInt(sorted[0][0]);
            const topOption = question.options.find(o => o.id === topOptionId);
            const topVotes = sorted[0][1];

            html += `
                <div class="popular-option">
                    <div class="popular-question">${question.title}</div>
                    <div class="popular-answer">${topOption.emoji} ${topOption.name} - ${topVotes} voto(s)</div>
                </div>
            `;
        }
    });
